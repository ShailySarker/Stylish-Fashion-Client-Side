...
const MenProducts = () => {
    ...
    useEffect(() => {
        const getMenProducts = async () => {
            try {
                ...
                setFilteredMenProducts(sortedProducts); // Initialize filteredProducts with all products
            } catch (error) {
    ...
    const prices = menProducts?.length ? menProducts?.map(product => product?.price) : [];
    ...
    useEffect(() => {
        ...
        //     filtered = filtered.filter((product) =>
        //         product?.color?.some((productColor) => selectedFilters?.color.includes(productColor))
        //     );
        // }
        if (selectedFilters?.color?.length > 0) {
            filtered = filtered.filter((product) =>
        ...
        setFilteredMenProducts(filtered);

    }, [selectedFilters, menProducts]);
