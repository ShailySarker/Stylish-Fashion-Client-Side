...
const DEMO_CREDENTIALS = {
  admin: { email: import.meta.env.VITE_ADMIN_EMAIL, password: import.meta.env.VITE_ADMIN_PASSWORD },
  user: { email: import.meta.env.VITE_USER_EMAIL, password: import.meta.env.VITE_USER_PASSWORD },
};

// Admin panel URL (update port if different)
...
const Login = () => {
  ...
  const handleLogin = async (event) => {
    ...
    if (result && result?.accessToken) {
      ...
      if (result.isAdmin) {
        ...
        window.location.replace(`${ADMIN_PANEL_URL}/home`);
      } else {
  ...
  const handleForgetPasswordWindow = async (e) => {
    ...
    if (!email) { alert("Kindly enter your email address!"); return; }
    const gmailRegex = /^[a-zA-Z0-9._-]+@gmail\.com$/;
    if (!gmailRegex.test(email)) { alert("Email is not in the correct format!"); return; }
    setIsLoading(true);
    try {
  ...
  const handleOTPVerify = async (event) => {
    ...
    if (otpCode?.length !== 5) { alert("Please enter the full 5-digit OTP"); return; }
  ...
  const handleOtpDigitChange = (index, value) => {
    if (/^\d*$/.test(value) && value.length <= 1) {
      ...
      if (index < otpDigits.length - 1 && value.length === 1) inputRefs.current[index + 1]?.focus();
      if (index > 0 && value.length === 0) inputRefs.current[index - 1]?.focus();
    }
  };
  ...
  const handleResendOTP = async () => {
    ...
    if (!userData?.email) { alert("User data not found. Please try again."); return; }
  ...
  const handleResetPasswordWindow = async (e) => {
    ...
    if (!/(?=.*[A-Z].*[A-Z])/.test(newPassword)) { alert("Please add at least two uppercase letters."); return; }
    if (!/(?=.*[0-9].*[0-9])/.test(newPassword)) { alert("Please add at least two numbers."); return; }
    if (!/(?=.*[!@#$&*])/.test(newPassword)) { alert("Please add a special character."); return; }
    if (newPassword.length < 8) { alert("Password must be at least 8 characters long."); return; }
    if (!newPassword || !confirmPassword) { alert("Please fill in all the required fields."); return; }
    if (newPassword !== confirmPassword) { alert("New password does not match with confirm password!"); return; }
    const userData = JSON.parse(localStorage.getItem("userData"));
    if (!userData?.email) { alert("User data not found. Please try again."); return; }
      ...
      <div className="lg:w-[45%] md:w-2/3 w-11/12 lg:mx-0 mx-auto">
        ...
        <div className="text-center w-[100%]">
          ...
          <div className="lg:mt-5 md:mt-4 mt-3">
            ...
            {isDemoFilling && (
              <p className="text-xs text-green-600 font-medium mt-1 animate-pulse">✓ Demo credentials filled — click Login!</p>
            )}
          </div>
          ...
          <div className="relative lg:px-10 lg:py-8 md:px-10 md:py-7 px-4 py-5 rounded-xl bg-white md:w-[560px] w-11/12 mx-auto shadow-lg">
            <div className="flex justify-between">
              ...
              <div className="w-[12%] flex justify-end">
                <FaX onClick={handleCloseModal} className="bg-purple-800 text-white p-1 md:w-7 md:h-7 w-5 h-5 rounded-full cursor-pointer" />
              </div>
            </div>
            ...
            <form onSubmit={handleForgetPasswordWindow} className="lg:mt-16 md:mt-10 mt-8">
              <div className="flex flex-col lg:gap-6 md:gap-5 gap-4 w-full">
                <div className="flex flex-col items-start lg:gap-2 gap-1 w-full">
                  ...
                  <input className="md:py-2 py-[6px] lg:px-5 md:px-4 px-3 rounded-xl w-full lg:text-lg shadow-lg border-2 border-purple-800" type="email" name="email" required />
                </div>
              </div>
              <button type="submit" className="lg:mt-14 md:mt-12 mt-8 bg-gradient-to-r from-blue-600 to-purple-800 text-white lg:w-44 md:w-40 w-36 px-4 lg:py-[10px] py-2 rounded-xl lg:text-xl md:text-lg text-base font-semibold shadow-lg mx-auto flex justify-center">
                Submit
              </button>
            </form>
        ...
        <div className="fixed inset-0 bg-[#ecf0f1bf] bg-opacity-75 overflow-y-auto flex items-center justify-center z-50">
          <div className="relative lg:px-10 lg:py-8 md:px-10 md:py-7 px-4 py-5 rounded-xl bg-white md:w-[560px] w-11/12 mx-auto shadow-lg">
            <div className="flex justify-between">
              ...
              <div className="w-[12%] flex justify-end">
                <FaX onClick={() => setOtpWindow(false)} className="bg-purple-800 text-white p-1 md:w-7 md:h-7 w-5 h-5 rounded-full cursor-pointer" />
              </div>
            </div>
            ...
            <form onSubmit={handleResetPasswordWindow} className="lg:mt-16 md:mt-10 mt-8">
              <div className="flex flex-col lg:gap-6 md:gap-5 gap-4 w-full">
                <div className="flex flex-col items-start lg:gap-2 gap-1 w-full">
                  ...
                  <div className="relative w-full">
                    <input type={newPasswordVisible ? "text" : "password"} className="md:py-2 py-[6px] lg:px-5 md:px-4 px-3 rounded-xl w-full lg:text-lg shadow-lg border-2 border-purple-800" name="newPassword" required />
                    <button type="button" className="absolute md:right-5 right-3 top-1/2 transform -translate-y-1/2 cursor-pointer" onClick={toggleNewPasswordVisibility}>
                ...
                <div className="flex flex-col items-start lg:gap-2 gap-1 w-full">
                  ...
                  <div className="relative w-full">
                    <input type={confirmPasswordVisible ? "text" : "password"} className="md:py-2 py-[6px] lg:px-5 md:px-4 px-3 rounded-xl w-full lg:text-lg shadow-lg border-2 border-purple-800" name="confirmPassword" required />
                    <button type="button" className="absolute md:right-5 right-3 top-1/2 transform -translate-y-1/2 cursor-pointer" onClick={toggleConfirmPasswordVisibility}>
