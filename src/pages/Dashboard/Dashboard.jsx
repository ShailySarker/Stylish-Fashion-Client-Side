...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (isAdmin) {
      const fetchAdminDashboardData = async () => {
        ...
        try {
          const [usersRes, ordersRes, userStatsRes, salesRes] =
            await Promise.all([
  ...
  const userOrderData = useMemo(() => {
    if (!orders || orders.length === 0)
      return months.map((m) => ({ name: m, orders: 0, spend: 0 }));
    const monthlyData = {};
    ...
    return months.map(
      (m) => monthlyData[m] || { name: m, orders: 0, spend: 0 },
    );
  }, [orders]);
    ...
    <div className="lg:px-20 md:px-12 px-6 py-10 min-h-screen bg-[#fcfcfd]">
      ...
      <div className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-center p-8 bg-white/40 backdrop-blur-md rounded-[40px] border border-white/60 shadow-[0_8px_32px_0_rgba(31,38,135,0.03)]">
        <div>
          ...
          <p className="text-gray-400 font-medium mt-1">
            Here is a summary of your platform pulse today.
          </p>
        </div>
        ...
        <div className="mt-6 md:mt-0 flex items-center gap-4">
          ...
          <div className="text-right">
            ...
            <p className="text-lg font-black text-gray-900">
              {isAdmin ? adminStats.totalOrders : orders.length}
            </p>
          </div>
        </div>
      </div>
        ...
        {isAdmin ? (
          <>
            ...
            <PremiumStatCard
              icon={<FaUserSecret />}
              label="Avg. Ticket"
              value={`$${(adminStats.totalRevenue / (adminStats.totalOrders || 1)).toFixed(0)}`}
              color="amber"
            />
          </>
        ) : (
          <>
            ...
            <PremiumStatCard
              ...
              value={`$${orders.reduce((acc, curr) => acc + curr.amount, 0).toLocaleString()}`}
              color="emerald"
            />
            <PremiumStatCard
              icon={<FaChartLine />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-12">
        ...
        <div className="bg-white p-10 rounded-[48px] border border-gray-100 shadow-[0_20px_50px_rgba(0,0,0,0.02)] group hover:shadow-xl transition-all duration-700">
          <div className="flex justify-between items-center mb-10">
            <div>
              <h3 className="text-xl font-black text-gray-900 tracking-tight">
                {isAdmin ? "Financial Performance" : "Investment Analysis"}
              </h3>
              <p className="text-xs text-gray-400 font-bold uppercase mt-1 tracking-widest">
                {isAdmin
                  ? "Revenue & Net Profit stream"
          ...
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={isAdmin ? formattedSalesStats : userOrderData}>
                ...
                />
                <Bar
                  dataKey={isAdmin ? "revenue" : "spend"}
                  fill="#3b82f6"
                  radius={[12, 12, 12, 12]}
                  barSize={isAdmin ? 12 : 24}
                />
                {isAdmin && (
                  <Bar
                    dataKey="profit"
      ...
      <div className="bg-white rounded-[40px] border border-gray-100 shadow-[0_20px_50px_rgba(0,0,0,0.02)] overflow-hidden">
        <div className="p-10 flex justify-between items-center border-b border-gray-50/50">
          <div>
            <h3 className="text-xl font-black text-gray-900 tracking-tight">
              {isAdmin ? "Live Platform Pulse" : "Recent Order Log"}
            </h3>
            <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mt-1 italic">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50/50">
              {(isAdmin ? adminStats.salesStats : orders)
                .slice(0, 5)
                .map((order, idx) => (
                  <tr
                    key={order._id || idx}
                  ...
                  >
                    <td className="px-10 py-6">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-purple-50 text-purple-600 flex items-center justify-center font-black italic">
                          #
                        </div>
                        <span className="text-sm font-black text-gray-900 italic">
                          TX-{order._id?.slice(-6) || "SYS001"}
                        </span>
                      </div>
                    </td>
...
const PremiumStatCard = ({ icon, label, value, trend, color }) => {
  ...
  return (
    <div
      className={`p-8 rounded-[40px] border bg-white shadow-sm flex flex-col gap-6 group hover:shadow-2xl hover:shadow-purple-500/5 transition-all duration-500`}
    >
      <div
        className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl transition-all duration-500 group-hover:scale-110 group-hover:rotate-12 ${colorClass}`}
      >
        {icon}
      </div>
      <div>
        ...
        <div className="flex items-end justify-between">
          <h4 className="text-3xl font-black text-gray-900 italic tracking-tighter">
            {value}
          </h4>
          {trend && (
